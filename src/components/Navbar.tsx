...
import { LogIn, Phone, UtensilsCrossed, Calendar, Menu, X, User } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {/* --- PROFILE OPTION --- */}
          {user ? (
            <Link to="/profile" className="flex items-center gap-3 pl-1 pr-4 py-1 bg-zinc-100 rounded-full hover:bg-black hover:text-white transition-all group">
              {user.photo ? (
                <img src={user.photo} alt="Profile" className="w-8 h-8 rounded-full object-cover border border-white" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-[#d4af37] text-white flex items-center justify-center font-bold text-xs">
                  {user.name.charAt(0).toUpperCase()}
                </div>
              )}
              <span className="text-[10px] font-bold uppercase tracking-widest hidden sm:block">My Profile</span>
            </Link>
          ) : (
            ...
            <div className="flex flex-col gap-8 text-center">
              {navLinks.map((link, i) => (
                <motion.div key={link.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + (i * 0.1) }}>
                  <Link to={link.path} className="text-4xl font-serif text-black hover:text-[#d4af37] transition-colors">{link.name}</Link>
                </motion.div>
              ))}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-8">
                {user ? (
                   <Link to="/profile" className="px-8 py-4 bg-zinc-100 rounded-full font-bold uppercase tracking-widest text-xs flex items-center gap-2">
